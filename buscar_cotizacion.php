...
<?php include "includes/wrapp.php"; ?>
			...
			<div class="articulo">
                ...
                <form action="buscar_cotizacion.php" method="get" class="form_search">
                    ...
                    <a href="lista_cotizacion.php"><img src="img/cerrar.png" class="btn_delete" style="margin-top:7px; margin-left:10px"></a>
                </form>
                ...
                <table>
					...
					$query= mysqli_query($conection,"SELECT c.id_cotizacion, c.fecha, c.fecha_inicio, c.fecha_final, p.nombre_responsable, p.nombre_comercial, p.rfc, p.calle, p.numero, p.id_colonia, asent.nombre1, c.id_persona, c.id_anuncio, a.ubicacion, a.tipo_anuncio, a.medida1, a.medida2, a.cantidad from personas p INNER JOIN cotizaciones_anuncio c on p.id_persona = c.id_persona INNER JOIN anuncios a on a.id_anuncio=c.id_anuncio INNER JOIN asentamientos asent on asent.id_asent = p.id_colonia where
                                    c.id_cotizacion like '%$busqueda%' or
                                    c.fecha like '%$busqueda%' or
                                    c.fecha_inicio like '%$busqueda%' or
                                    c.fecha_final like '%$busqueda%' or
                                    ...
                                    p.id_colonia like '%$busqueda%' or
                                    asent.nombre1 like '%$busqueda%' or
                                    c.id_anuncio like '%$busqueda%' or
                                    a.tipo_anuncio like '%$busqueda%' or
                                    a.medida1 like '%$busqueda%' or
                                    a.medida2 like '%$busqueda%' or
                                    a.cantidad like '%$busqueda%' 

                                    order by id_cotizacion asc LIMIT $desde,$por_pagina");
					mysqli_close($conection);
